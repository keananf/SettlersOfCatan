...
public class TradeTests extends TestHelper
{
	...
	public void cannotAffordPortTradeTest()
			throws IllegalTradeException, IllegalPortTradeException, CannotAffordException
	{
		Port port = game.getGrid().ports.get(0);
		PortTradeProto.Builder portTrade = setUpPortTrade(port, new HashMap<ResourceType, Integer>(),
	...
	{
		Port port = game.getGrid().ports.get(0);
		ResourceType receiveType = port.getReturnType() == ResourceType.Generic ? ResourceType.Brick
				: port.getReturnType();
		ResourceType exchangeType = port.getExchangeType() == ResourceType.Generic ? ResourceType.Lumber
				: port.getExchangeType();
	...
	private PortTradeProto.Builder setUpPortTrade(Port port, Map<ResourceType, Integer> grant,
			Map<ResourceType, Integer> requestMap)
	{
		...
		case Grain:
			offer.setGrain(
					grant.containsKey(ResourceType.Grain) ? grant.get(ResourceType.Grain) : port.getReturnAmount());
			break;
		case Brick:
			offer.setBrick(
					grant.containsKey(ResourceType.Brick) ? grant.get(ResourceType.Brick) : port.getReturnAmount());
			break;
		case Generic:
		case Lumber:
			offer.setLumber(
					grant.containsKey(ResourceType.Lumber) ? grant.get(ResourceType.Lumber) : port.getReturnAmount());
			break;
		}
		switch (port.getReturnType())
		{
		case Wool:
			request.setWool(requestMap.containsKey(ResourceType.Wool) ? requestMap.get(ResourceType.Wool)
					: port.getReturnAmount());
			break;
		case Ore:
			request.setOre(requestMap.containsKey(ResourceType.Ore) ? requestMap.get(ResourceType.Ore)
					: port.getReturnAmount());
			break;
		case Grain:
			request.setGrain(requestMap.containsKey(ResourceType.Grain) ? requestMap.get(ResourceType.Grain)
					: port.getReturnAmount());
			break;
		case Generic:
		case Brick:
			request.setBrick(requestMap.containsKey(ResourceType.Brick) ? requestMap.get(ResourceType.Brick)
					: port.getReturnAmount());
			break;
		case Lumber:
			request.setLumber(requestMap.containsKey(ResourceType.Lumber) ? requestMap.get(ResourceType.Lumber)
					: port.getReturnAmount());
			break;
		}
